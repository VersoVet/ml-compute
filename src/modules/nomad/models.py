...
class NomadJobRequest(BaseModel):
    ...
    job_type: str = Field(default="batch", description="Job type: batch, service, system, sysbatch")
    ...
    image: str | None = Field(default=None, description="Docker image (for Docker driver)")
    driver: str = Field(default="docker", description="Driver: docker, raw_exec, exec")
    env_vars: dict[str, str] = Field(default_factory=dict, description="Environment variables")
    volumes: dict[str, str] = Field(default_factory=dict, description="Host path -> container path mapping")
...
class NomadJobStatus(BaseModel):
    ...
    allocations: list[AllocationStatus] = Field(default_factory=list, description="Task allocations")
...
class GPUStatus(BaseModel):
    ...
    in_use: list[str] = Field(default_factory=list, description="Job IDs using GPUs")
